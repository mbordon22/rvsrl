        ...
        return (new EloquentDataTable($query))
            ->editColumn('estado', function ($row) {
                return view('admin.inc.action', [
                    'toggle' => $row,
                    'name' => 'estado',
                    'route' => 'admin.inventarios.materiales.status',
                    'value' => $row->estado,
                    'permission' => auth()->user()->can('listado_materiales.edit')
                ]);
            })
            ->editColumn('action', function ($row) {
                $acciones = ['data' => $row];
                if (auth()->user()->can('listado_materiales.edit')) {
                    $acciones['edit'] = 'admin.inventarios.materiales.edit';
                }
                if (auth()->user()->can('listado_materiales.trash')) {
                    $acciones['delete'] = 'admin.inventarios.materiales.destroy';
                }
