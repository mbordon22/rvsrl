...
$(document).on('click', '#add-centro-costo-btn', function (e) {
    e.preventDefault();
    ...
    $.ajax({
        ...
        success: function (response) {
            $('#centrocosto-table').DataTable().ajax.reload(null, false);
            ...
            swal({
                ...
                $('#addCentroCostoModal').modal('hide');
            });
        },
        error: function (xhr) {
...
$(document).on('click', '.edit-btn', function () {
    ...
    const url = `{{ url('admin/gestion-contable/centro-costo') }}/${id}`;
    
    $.get(url, function (data) {
        $('#edit-nombre').val(data.nombre);
        $('#edit-descripcion').val(data.descripcion);
        $('#edit-id').val(data.id);
        $('#edit-centro-costo-form').attr('action', `{{ url('admin/gestion-contable/centro-costo') }}/${id}`);
        $('#editCentroCostoModal').modal('show');
    });
});

//Editar un registro
$(document).on('click', '#edit-centro-costo-btn', function (e) {
    e.preventDefault();
    e.target.disabled = true; // Deshabilitar el botón para evitar múltiples envíos
    e.target.innerHTML = 'Cargando...'; // Cambiar el texto del botón
    ...
    $.ajax({
        ...
        success: function (response) {
            ...
            swal({
                ...
                $('#editCentroCostoModal').modal('hide');
            });
        },
        error: function (xhr) {
            const response = JSON.parse(xhr.responseText);
...
$(document).on('click', '.btn-delete', function() {
    ...
    swal({
        ...
        if (willDelete) {
            $.ajax({
                url: `/admin/gestion-contable/centro-costo/${id}`,
                type: 'DELETE',
                data: {
                    _token: '{{ csrf_token() }}'
                },
