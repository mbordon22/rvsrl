...
$(document).on('click', '#add-documento-btn', function (e) {
    e.preventDefault();
    ...
    $.ajax({
        ...
        success: function (response) {
            $('#vehiculodoc-table').DataTable().ajax.reload(null, false);
        ...
        complete: function () {
            ...
            e.target.innerHTML = 'Añadir Documento'; // Restaurar el texto del botón
        }
    });
});
...
$(document).on('click', '.edit-document-btn', function () {
    ...
    const url = `{{ url('admin/vehiculos/documentacion') }}/${id}`;
...
//Editar un registro
$(document).on('click', '#edit-documento-btn', function (e) {
    e.preventDefault();
    e.target.disabled = true; // Deshabilitar el botón para evitar múltiples envíos
    e.target.innerHTML = 'Cargando...'; // Cambiar el texto del botón
...
$(document).on('click', '.btn-delete', function() {
    ...
    swal({
        ...
        if (willDelete) {
            $.ajax({
                url: `/admin/vehiculos/documentacion/${id}`,
                type: 'DELETE',
                data: {
                    _token: '{{ csrf_token() }}'
                },
...
$(document).on('click', '.view-file-btn', function () {
    ...
    $.get(url, function (data) {
        $('#viewFilesDocumentModalLabel').text(data.tipo_documento);
        ...
        $('#viewFilesDocumentModal').modal('show');
    });
});
</script>